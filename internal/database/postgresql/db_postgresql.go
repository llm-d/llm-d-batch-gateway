...
func newPool(ctx context.Context, config *PostgreSQLConfig) (*pgxpool.Pool, error) {
	...
	// The event client reserves a connection in WaitForNotification while it
	// also queries the durable event table. Keep at least one connection free
	// for those queries when callers set MaxConns to one.
